// Onboard the 10 fresh Google-Play habit apps into time-tracking, replacing the
...
const metaList = JSON.parse(fs.readFileSync("/tmp/time/meta.json", "utf8"));
...
cat.apps = [];
if (cat.ru) cat.ru.kicker = "Toggl, Focus To-Do, aTimeLogger — учёт времени и фокус";
if (cat.en) cat.en.kicker = "Toggl, Focus To-Do, aTimeLogger — time tracking and focus";

for (const m of metaList) {
  const pid = m.pid;
  const out = JSON.parse(fs.readFileSync(`/tmp/time/out/${pid}.json`, "utf8"));
...
console.log("time-tracking roster now:", cat.apps.length, "apps");
