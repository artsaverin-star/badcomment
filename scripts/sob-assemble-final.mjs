...
//  /tmp/sob/summary.json  (category clusters with cardIdx into pool)
//  /tmp/sob/pool.json     (app cards with quotes)
//  /tmp/sob/ideas-out.json(ideas with mechIdx into summary.product)
...
const pool = J("/tmp/sob/pool.json").cards;
const byIdx = new Map(pool.map((c) => [c.i, c]));
const summary = J("/tmp/sob/summary.json").product || [];
const seg = J(path.join(ROOT, "src/data/segment-insights.json")).sobriety || {};
...
for (const it of ideasOut) {
  ...
  ideas.push({
    slug: `sobriety-${n}`,
    category: "sobriety",
    categoryName: "Бросить пить, курить, скроллить",
...
console.log(JSON.stringify({ product: product.length, hygiene: hygiene.length, ideas: ideas.length }));
