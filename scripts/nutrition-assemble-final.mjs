...
//  /tmp/nutrition/summary.json  (category clusters with cardIdx into pool)
//  /tmp/nutrition/pool.json     (app cards with quotes)
//  /tmp/nutrition/ideas-out.json(ideas with mechIdx into summary.product)
...
const pool = J("/tmp/nutrition/pool.json").cards;
const byIdx = new Map(pool.map((c) => [c.i, c]));
const summary = J("/tmp/nutrition/summary.json").product || [];
const seg = J(path.join(ROOT, "src/data/segment-insights.json"))['nutrition-calories'] || {};
...
segCards['nutrition-calories'] = { product, hygiene };
fs.writeFileSync(path.join(ROOT, "src/data/segment-cards.json"), JSON.stringify(segCards));

// Ideas
const ideasOut = J("/tmp/nutrition/ideas-out.json").ideas || [];
const ideas = [];
let n = 0;
for (const it of ideasOut) {
  ...
  n++;
  ideas.push({
    slug: `nutrition-calories-${n}`,
    category: "nutrition-calories",
    categoryName: "Калории и питание",
...
// Merge: keep other categories' ideas, replace nutrition-calories ones.
const existing = J(path.join(ROOT, "src/data/ideas.json"));
const merged = existing.filter((i) => i.category !== "nutrition-calories").concat(ideas);
