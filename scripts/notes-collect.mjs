...
// /tmp/notes/src/<key>.json + /tmp/notes/collect-meta.json.
import gplayPkg from "google-play-scraper";
const gplay = gplayPkg.default || gplayPkg;
import fs from "node:fs";

const APPS = [
  { key: "notion", pkg: "notion.id" },
  { key: "obsidian", pkg: "md.obsidian" },
  { key: "evernote", pkg: "com.evernote" },
  { key: "onenote", pkg: "com.microsoft.office.onenote" },
  { key: "keep", pkg: "com.google.android.keep" },
  { key: "samsung", pkg: "com.samsung.android.app.notes" },
  { key: "colornote", pkg: "com.socialnmobile.dictapps.notepad.color.note" },
  { key: "simplenote", pkg: "com.automattic.simplenote" },
  { key: "wenote", pkg: "com.yocto.wenote" },
  { key: "zoho", pkg: "com.zoho.notebook" },
];

const COUNTRIES = ["us", "gb", "in", "ca", "au"];
const LANGS = ["en", "es", "pt", "de", "fr", "it"];
const OUT = "/tmp/notes/src";
...
fs.writeFileSync("/tmp/notes/collect-meta.json", JSON.stringify(results, null, 2));
console.log("DONE");
