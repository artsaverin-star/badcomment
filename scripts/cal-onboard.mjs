// Onboard the 10 fresh Google-Play habit apps into calendars-tasks, replacing the
...
const SLUG = "calendars-tasks";

const metaList = JSON.parse(fs.readFileSync("/tmp/cal/meta.json", "utf8"));
...
cat.apps = [];
if (cat.ru) cat.ru.kicker = "Todoist, TickTick, Google Calendar — то-ду и календари";
if (cat.en) cat.en.kicker = "Todoist, TickTick, Google Calendar — to-do and calendars";

for (const m of metaList) {
  const pid = m.pid;
  const out = JSON.parse(fs.readFileSync(`/tmp/cal/out/${pid}.json`, "utf8"));
...
console.log("calendars-tasks roster now:", cat.apps.length, "apps");
