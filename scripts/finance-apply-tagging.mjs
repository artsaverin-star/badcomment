...
const meta = JSON.parse(fs.readFileSync("/tmp/finance/meta.json", "utf8"));
const name = {};
for (const m of meta) name[m.productId || `ext-${m.appleId}`] = m.name;

const flat = [...seg['personal-finance'].product, ...seg['personal-finance'].hygiene]; // id = index
const byCardEvidence = flat.map(() => []);

for (const f of fs.readdirSync("/tmp/finance/classify-out").filter((x) => x.endsWith(".json"))) {
  const pid = f.replace(/\.json$/, "");
  let out;
  try { out = JSON.parse(fs.readFileSync(`/tmp/finance/classify-out/${f}`, "utf8")); } catch { continue; }
  const revs = JSON.parse(fs.readFileSync(`/tmp/finance/src/${pid}.json`, "utf8"));
