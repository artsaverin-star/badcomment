...
const meta = JSON.parse(fs.readFileSync("/tmp/notes/meta.json", "utf8"));
const name = {};
for (const m of meta) name[m.productId || `ext-${m.appleId}`] = m.name;

const flat = [...seg['notes-pkm'].product, ...seg['notes-pkm'].hygiene]; // id = index
const byCardEvidence = flat.map(() => []);

for (const f of fs.readdirSync("/tmp/notes/classify-out").filter((x) => x.endsWith(".json"))) {
  const pid = f.replace(/\.json$/, "");
  let out;
  try { out = JSON.parse(fs.readFileSync(`/tmp/notes/classify-out/${f}`, "utf8")); } catch { continue; }
  const revs = JSON.parse(fs.readFileSync(`/tmp/notes/src/${pid}.json`, "utf8"));
