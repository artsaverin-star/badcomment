...
const meta = JSON.parse(fs.readFileSync("/tmp/time/meta.json", "utf8"));
const name = {};
for (const m of meta) name[m.productId || `ext-${m.appleId}`] = m.name;

const flat = [...seg['time-tracking'].product, ...seg['time-tracking'].hygiene]; // id = index
const byCardEvidence = flat.map(() => []);

for (const f of fs.readdirSync("/tmp/time/classify-out").filter((x) => x.endsWith(".json"))) {
  const pid = f.replace(/\.json$/, "");
  let out;
  try { out = JSON.parse(fs.readFileSync(`/tmp/time/classify-out/${f}`, "utf8")); } catch { continue; }
  const revs = JSON.parse(fs.readFileSync(`/tmp/time/src/${pid}.json`, "utf8"));
