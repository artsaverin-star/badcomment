...
const meta = J("/tmp/nutrition/meta.json");
const name = {};
for (const m of meta) name[m.productId || `ext-${m.appleId}`] = m.name;
const appCards = J(path.join(ROOT, "src/data/app-cards.json"));
let done = 0;
for (const f of fs.readdirSync("/tmp/nutrition/out2").filter((x) => x.endsWith(".json"))) {
  const pid = f.replace(/\.json$/, "");
  let out, revs;
  try { out = J(`/tmp/nutrition/out2/${f}`); revs = J(`/tmp/nutrition/src/${pid}.json`); } catch { continue; }
