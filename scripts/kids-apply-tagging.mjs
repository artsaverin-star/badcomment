...
const meta = JSON.parse(fs.readFileSync("/tmp/kids/meta.json", "utf8"));
const name = {};
for (const m of meta) name[m.productId || `ext-${m.appleId}`] = m.name;

const flat = [...seg['kids-learning'].product, ...seg['kids-learning'].hygiene]; // id = index
const byCardEvidence = flat.map(() => []);

for (const f of fs.readdirSync("/tmp/kids/classify-out").filter((x) => x.endsWith(".json"))) {
  const pid = f.replace(/\.json$/, "");
  let out;
  try { out = JSON.parse(fs.readFileSync(`/tmp/kids/classify-out/${f}`, "utf8")); } catch { continue; }
  const revs = JSON.parse(fs.readFileSync(`/tmp/kids/src/${pid}.json`, "utf8"));
