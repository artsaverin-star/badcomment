...
//    in the article and the locked preview, the old archive bridge) and the sitemap, in-process
...
//    first in the TOC, before the paywall when locked) and the old review hub; on each host page
//    (topic, archive category, archive hub, dossier) the block's reviews and apps equal the
//    page's own. Skipped when no server answers.
...
describe("rendered HTML", () => {
  ...
  describe("server components", () => {
    ...
    test("old archive bridge: the same block, real scores, links via publicHref, nothing for a category without needs", async () => {
      // The hub prints the archive's totals above the block; the block repeats them.
      const summary = m.reviews.totals();
      const hub = render(await m.LegacyPulseLink({ locale: "ru", stats: { reviewCount: summary.sourceReviews, appCount: summary.sourceApps } }));
      const ru = pulseStrings.ru;
      assert.ok(hub.includes('href="/ru/pulse"'));
      assert.ok(textOf(hub).includes(`${needsPhrase("ru", ru, needs.length)} · ${aboutAppsPhrase("ru", ru, summary.sourceReviews, summary.sourceApps)}`));
      assert.ok(separatorsGlued(hub));
      assert.ok(textOf(hub).includes(allNeedsPhrase("ru", ru, needs.length)));
      assert.deepEqual(ticks(hub), { all: 10, on: needs[0].score, needles: 1 });
      assert.equal(count(hub, /class="ia-pulse-barrow"/g), Math.min(PULSE_EMBED_LIMIT, needs.length) - 1);
      // Cross-category: each row and the lead name their category.
      assert.equal(count(hub, /class="ia-pulse-barrow__meta"/g), Math.min(PULSE_EMBED_LIMIT, needs.length) - 1);
      const names = new Map(data.categories.map((c) => [c.id, c.name.ru || c.name.en]));
      assert.ok(textOf(hub).includes(`${names.get(needs[0].categoryId)}. ${needAria("ru", ru, needs[0])}`));
      assert.ok(!KIND_MARKUP.test(hub));
      for (const word of KIND_WORDS) assert.ok(!textOf(hub).includes(word), word);
      for (const href of hrefs(hub).filter((h) => h !== "/ru/pulse")) assert.equal(resolvePulseRoute(href.replace("/ru/pulse/", ""), data).type, "need", href);
      for (const id of categoryIds) {
        // /reviews/<slug> prints «100 apps · 46,072 reviews» (the catalogue's sourceReviews and the
        // source apps); the dossier prints reviewNicheTotals — the same figures.
        const niche = m.reviews.getNiche(id);
        const archive = { reviewCount: (niche?.sourceReviews || niche?.apps.reduce((sum, app) => sum + app.total, 0)) ?? 0, appCount: m.reviews.listSourceApps(id).length };
        ...
        const html = render(await m.LegacyPulseLink({ locale: "en", slug: id, stats: archive }));
        const own = categoryNeeds(data, id);
        assert.ok(textOf(html).includes(aboutAppsPhrase("en", pulseStrings.en, archive.reviewCount, archive.appCount)), `${id}: the page's figures`);
...
describe(`live HTTP (${BASE})`, { timeout: 600_000 }, () => {
  ...
  test("the block's reviews and apps are the host page's own: topic, archive category, archive hub, dossier", async (t) => {
    ...
    // The archive: «100 приложений · 46 072 отзывов» under the h1 of /reviews/<slug>, and the hub's
    // «71 категорий · 4 623 приложения · 1 451 072 отзывов».
    const slug = withNeeds.has("dating-apps") ? "dating-apps" : categoryIds[0];
    for (const locale of ["ru", "en"] as const) {
      const res = await get(`/${locale}/reviews/${slug}`);
      if (res.status !== 200) continue;
      const html = await res.text();
      const [apps, reviews] = nums(textOf(/<\/h1>\s*<p[^>]*>([\s\S]*?)<\/p>/.exec(html)?.[1] ?? ""));
      assert.deepEqual(block(html).slice(1), [reviews, apps], `/${locale}/reviews/${slug}`);
      checked.push(`/${locale}/reviews/${slug}`);
    }
    const hubRes = await get(`/ru/reviews`);
    if (hubRes.status === 200) {
      const html = await hubRes.text();
      const header = /<header>[\s\S]*?<\/header>/.exec(html)?.[0] ?? "";
      const [, apps, reviews] = nums(textOf([...header.matchAll(/<p[^>]*>([\s\S]*?)<\/p>/g)].at(-1)?.[1] ?? ""));
      assert.deepEqual(block(html).slice(1), [reviews, apps], "/ru/reviews");
      checked.push("/ru/reviews");
    }
  ...
  test("old review hub and a category page carry the block, with no kind label", async (t) => {
    if (!(await needServer(t))) return;
    const html = await (await get(`/ru/reviews`)).text();
    assert.ok(html.includes('href="/ru/pulse"'));
    assert.ok(needLinks(html, "ru").length > 0);
    assert.equal(count(html, /class="ia-pulse-hero/g) > 0, true);
    const slug = withNeeds.has("dating-apps") ? "dating-apps" : categoryIds[0];
    const page = await get(`/ru/reviews/${slug}`);
    if (page.status === 200) {
