...
//  /tmp/finance/summary.json  (category clusters with cardIdx into pool)
//  /tmp/finance/pool.json     (app cards with quotes)
//  /tmp/finance/ideas-out.json(ideas with mechIdx into summary.product)
...
const pool = J("/tmp/finance/pool.json").cards;
const byIdx = new Map(pool.map((c) => [c.i, c]));
const summary = J("/tmp/finance/summary.json").product || [];
const seg = J(path.join(ROOT, "src/data/segment-insights.json"))['personal-finance'] || {};
...
segCards['personal-finance'] = { product, hygiene };
fs.writeFileSync(path.join(ROOT, "src/data/segment-cards.json"), JSON.stringify(segCards));

// Ideas
const ideasOut = J("/tmp/finance/ideas-out.json").ideas || [];
const ideas = [];
let n = 0;
for (const it of ideasOut) {
  ...
  n++;
  ideas.push({
    slug: `personal-finance-${n}`,
    category: "personal-finance",
    categoryName: "Личные финансы",
...
// Merge: keep other categories' ideas, replace personal-finance ones.
const existing = J(path.join(ROOT, "src/data/ideas.json"));
const merged = existing.filter((i) => i.category !== "personal-finance").concat(ideas);
