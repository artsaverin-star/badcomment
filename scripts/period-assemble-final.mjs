...
//  /tmp/period/summary.json  (category clusters with cardIdx into pool)
//  /tmp/period/pool.json     (app cards with quotes)
//  /tmp/period/ideas-out.json(ideas with mechIdx into summary.product)
...
const pool = J("/tmp/period/pool.json").cards;
const byIdx = new Map(pool.map((c) => [c.i, c]));
const summary = J("/tmp/period/summary.json").product || [];
const seg = J(path.join(ROOT, "src/data/segment-insights.json"))['period-cycle'] || {};
...
segCards['period-cycle'] = { product, hygiene };
fs.writeFileSync(path.join(ROOT, "src/data/segment-cards.json"), JSON.stringify(segCards));

// Ideas
const ideasOut = J("/tmp/period/ideas-out.json").ideas || [];
const ideas = [];
let n = 0;
for (const it of ideasOut) {
  ...
  ideas.push({
    ...
    categoryName: "Месячные и фертильность",
...
// Merge: keep other categories' ideas, replace period-cycle ones.
const existing = J(path.join(ROOT, "src/data/ideas.json"));
const merged = existing.filter((i) => i.category !== "period-cycle").concat(ideas);
