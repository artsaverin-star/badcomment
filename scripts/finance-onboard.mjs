// Onboard the 10 fresh Google-Play habit apps into personal-finance, replacing the
...
const metaList = JSON.parse(fs.readFileSync("/tmp/finance/meta.json", "utf8"));
...
cat.apps = [];
if (cat.ru) cat.ru.kicker = "Money Manager, Wallet, Monefy — бюджеты и учёт расходов";
if (cat.en) cat.en.kicker = "Money Manager, Wallet, Monefy — budgets and expense tracking";

for (const m of metaList) {
  const pid = m.pid;
  const out = JSON.parse(fs.readFileSync(`/tmp/finance/out/${pid}.json`, "utf8"));
...
console.log("personal-finance roster now:", cat.apps.length, "apps");
