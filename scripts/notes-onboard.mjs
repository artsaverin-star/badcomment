// Onboard the 10 fresh Google-Play habit apps into notes-pkm, replacing the
...
const SLUG = "notes-pkm";

const metaList = JSON.parse(fs.readFileSync("/tmp/notes/meta.json", "utf8"));
...
cat.apps = [];
if (cat.ru) cat.ru.kicker = "Notion, Obsidian, Evernote — заметки и база знаний";
if (cat.en) cat.en.kicker = "Notion, Obsidian, Evernote — notes and knowledge base";

for (const m of metaList) {
  const pid = m.pid;
  const out = JSON.parse(fs.readFileSync(`/tmp/notes/out/${pid}.json`, "utf8"));
...
console.log("notes-pkm roster now:", cat.apps.length, "apps");
