// Onboard the 10 fresh Google-Play habit apps into habit-tracking, replacing the
...
const SLUG = "habit-tracking";

const metaList = JSON.parse(fs.readFileSync("/tmp/habit/meta.json", "utf8"));
...
cat.apps = [];
if (cat.ru) cat.ru.kicker = "Habitica, Finch, Loop — серии дней и геймификация";
if (cat.en) cat.en.kicker = "Habitica, Finch, Loop — streaks and gamification";

for (const m of metaList) {
  const pid = m.pid;
  const out = JSON.parse(fs.readFileSync(`/tmp/habit/out/${pid}.json`, "utf8"));
...
console.log("habit-tracking roster now:", cat.apps.length, "apps");
