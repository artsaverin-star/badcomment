...
//   AUTHOR_SCREEN=1     compact rows (6 short cons / 2 pros) — cheap classify
//                       pass; re-fetch full data for confirmed gems
//   AUTHOR_IDS=a,b,c    only these ids, full data (author the confirmed gems)
...
async function run() {
  ...
  const idSet = (process.env.AUTHOR_IDS ?? "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  let feed = cards.map((c) => ({
    ...
    cons: screen ? c.conSamples.slice(0, 6).map((t) => trunc(t, 120)) : c.conSamples,
    pros: screen ? c.proSamples.slice(0, 2).map((t) => trunc(t, 100)) : c.proSamples,
  }));

  if (onlyNew) feed = feed.filter((c) => !c.authored);
  if (idSet.length) feed = feed.filter((c) => idSet.includes(c.id));
  if (offset > 0 && !idSet.length) feed = feed.slice(offset);
  if (limit > 0 && !idSet.length) feed = feed.slice(0, limit);
