// Onboard the 10 fresh Google-Play habit apps into kids-learning, replacing the
...
const SLUG = "kids-learning";

const metaList = JSON.parse(fs.readFileSync("/tmp/kids/meta.json", "utf8"));
...
cat.apps = [];
if (cat.ru) cat.ru.kicker = "Khan Academy Kids, ABCmouse, Epic — раннее развитие и чтение";
if (cat.en) cat.en.kicker = "Khan Academy Kids, ABCmouse, Epic — early learning & reading";

for (const m of metaList) {
  const pid = m.pid;
  const out = JSON.parse(fs.readFileSync(`/tmp/kids/out/${pid}.json`, "utf8"));
...
console.log("kids-learning roster now:", cat.apps.length, "apps");
