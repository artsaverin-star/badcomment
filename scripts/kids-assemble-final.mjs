...
//  /tmp/kids/summary.json  (category clusters with cardIdx into pool)
//  /tmp/kids/pool.json     (app cards with quotes)
//  /tmp/kids/ideas-out.json(ideas with mechIdx into summary.product)
...
const pool = J("/tmp/kids/pool.json").cards;
const byIdx = new Map(pool.map((c) => [c.i, c]));
const summary = J("/tmp/kids/summary.json").product || [];
const seg = J(path.join(ROOT, "src/data/segment-insights.json"))['kids-learning'] || {};
...
segCards['kids-learning'] = { product, hygiene };
fs.writeFileSync(path.join(ROOT, "src/data/segment-cards.json"), JSON.stringify(segCards));

// Ideas
const ideasOut = J("/tmp/kids/ideas-out.json").ideas || [];
const ideas = [];
let n = 0;
for (const it of ideasOut) {
  ...
  n++;
  ideas.push({
    slug: `kids-learning-${n}`,
    category: "kids-learning",
    categoryName: "Обучение для детей",
...
// Merge: keep other categories' ideas, replace kids-learning ones.
const existing = J(path.join(ROOT, "src/data/ideas.json"));
const merged = existing.filter((i) => i.category !== "kids-learning").concat(ideas);
