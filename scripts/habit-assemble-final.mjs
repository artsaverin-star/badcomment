...
//  /tmp/habit/summary.json  (category clusters with cardIdx into pool)
//  /tmp/habit/pool.json     (app cards with quotes)
//  /tmp/habit/ideas-out.json(ideas with mechIdx into summary.product)
...
const pool = J("/tmp/habit/pool.json").cards;
const byIdx = new Map(pool.map((c) => [c.i, c]));
const summary = J("/tmp/habit/summary.json").product || [];
const seg = J(path.join(ROOT, "src/data/segment-insights.json"))['habit-tracking'] || {};
...
segCards['habit-tracking'] = { product, hygiene };
fs.writeFileSync(path.join(ROOT, "src/data/segment-cards.json"), JSON.stringify(segCards));

// Ideas
const ideasOut = J("/tmp/habit/ideas-out.json").ideas || [];
const ideas = [];
let n = 0;
for (const it of ideasOut) {
  ...
  n++;
  ideas.push({
    slug: `habit-tracking-${n}`,
    category: "habit-tracking",
    categoryName: "Привычки и стрики",
...
// Merge: keep other categories' ideas, replace habit-tracking ones.
const existing = J(path.join(ROOT, "src/data/ideas.json"));
const merged = existing.filter((i) => i.category !== "habit-tracking").concat(ideas);
