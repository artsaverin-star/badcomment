...
//  /tmp/time/summary.json  (category clusters with cardIdx into pool)
//  /tmp/time/pool.json     (app cards with quotes)
//  /tmp/time/ideas-out.json(ideas with mechIdx into summary.product)
...
const pool = J("/tmp/time/pool.json").cards;
const byIdx = new Map(pool.map((c) => [c.i, c]));
const summary = J("/tmp/time/summary.json").product || [];
const seg = J(path.join(ROOT, "src/data/segment-insights.json"))['time-tracking'] || {};
...
segCards['time-tracking'] = { product, hygiene };
fs.writeFileSync(path.join(ROOT, "src/data/segment-cards.json"), JSON.stringify(segCards));

// Ideas
const ideasOut = J("/tmp/time/ideas-out.json").ideas || [];
const ideas = [];
let n = 0;
for (const it of ideasOut) {
  ...
  n++;
  ideas.push({
    slug: `time-tracking-${n}`,
    category: "time-tracking",
    categoryName: "Учёт времени",
...
// Merge: keep other categories' ideas, replace time-tracking ones.
const existing = J(path.join(ROOT, "src/data/ideas.json"));
const merged = existing.filter((i) => i.category !== "time-tracking").concat(ideas);
