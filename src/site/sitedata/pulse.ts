 ...
 * previous analysis served in place at /<ru|en>/segment/<slug>, else the review archive of the
 * category (both old pages: plain <a>). Null when the site has nothing for it.
 */
export function pulseCategoryHref(locale: Locale, slug: string): { href: string; oldSite: boolean } | null {
  ...
  const old = toOldLocale(locale);
  if (oldTopicPages.has(slug)) return { href: `/${old}/segment/${slug}`, oldSite: true };
  if (oldReviewHubs.has(slug)) return { href: `/${old}/reviews/${slug}`, oldSite: true };
  return null;
}
