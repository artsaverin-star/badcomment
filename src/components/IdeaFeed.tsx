...
export default function IdeaFeed({
  ...
  fullDeck = true, onLockedOpen, compact = false,
}: {
  ...
  // Compact embedding (homepage / niche pages) — shorter so the section fits a
  // small screen; the dedicated /cards page stays tall.
  compact?: boolean;
}) {
  const ru = locale !== "en";
  ...
  return (
    <div className="mx-auto flex w-full max-w-[480px] flex-col items-center sm:max-w-[700px]">
      <div className="relative w-full">
        ...
        <div
          className={`feed-mask relative ${VIEWPORT_H} w-full select-none`}
        ...
        >
          ...
          {idx === 0 && (
            <div aria-hidden className="pointer-events-none absolute left-1/2 top-1/2 w-[296px] opacity-40" style={{ transform: `translate(-50%, -50%) translateX(${-SLOT + dragX}px)`, transition }}>
...
function Ruba() {
  return (
    <div className="flip-face overflow-hidden rounded-[24px] border border-white/15 p-2 shadow-[0_28px_70px_-30px_rgba(0,0,0,0.85)] grayscale" style={{ backgroundImage: "linear-gradient(135deg,#FF5C8A 0%,#B14DEA 50%,#4CB8F5 100%)" }}>
