...
// bottom of the card. Keeps secondary facts tidy instead of stacked color blocks.
...
export default function IdeaCardExploded({
  card,
  locale,
}: {
  card: IdeaCard;
  ...
  return (
    <article className="mx-auto flex w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-black/10 bg-white dark:border-white/10 dark:bg-neutral-900">
      {/* ── Identity ───────────────────────────────── */}
      <div className="flex flex-col gap-3 p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {s?.tagline && (
              <p className="mt-0.5 text-sm text-neutral-600 dark:text-neutral-300">{s.tagline}</p>
            )}
      ...
      {/* Screenshots strip from the store. Fixed phone-frame thumbnails so the
          row stays uniform across portrait/landscape source shots (cover from
          the top keeps each screenshot's meaningful header instead of letterboxing). */}
      {card.screenshots.length > 0 && (
        <div className="flex gap-3 overflow-x-auto px-5 pb-4">
          {card.screenshots.map((src) => (
            // eslint-disable-next-line @next/next/no-img-element
            <img
              key={src}
              src={src}
              alt=""
              className="h-72 w-36 shrink-0 rounded-xl border border-black/10 object-cover object-top dark:border-white/10"
            />
          ))}
        </div>
      )}
      ...
      {s && s.cloneable === false && s.buildNote && (
        <div className="px-5 pb-4">
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm dark:border-amber-900 dark:bg-amber-950/30">
            <p className="font-semibold text-amber-700 dark:text-amber-400">{tr.card.cantClone}</p>
