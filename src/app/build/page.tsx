...
export async function generateMetadata() {
  ...
  const ru = locale !== "en";
  const lp = ru ? "ru" : "en";
  const url = `https://inapp.pro/${lp}/build`;
  const title = ru ? "Собери своё приложение" : "Build your own app";
  const description = ru
    ? "Выбираешь проверенную боль из реальных отзывов, получаешь план и собираешь приложение за вечер с любой нейросетью."
    : "Pick a pain verified by real reviews, get the plan and build the app in an evening with whatever AI you use.";
  return {
    title,
    description,
    alternates: {
      canonical: url,
      languages: { ru: "https://inapp.pro/ru/build", en: "https://inapp.pro/en/build", "x-default": "https://inapp.pro/en/build" },
    },
...
// «Создание» — вход в конструктор: что здесь происходит, почему это работает и
// какую суперсилу получает пользователь. Шаг 1 дороги (выбор ниши) живёт прямо
// тут. Жил на корне 11–16 июля; главная снова «Разборы» — туда шли и клики, и
// платящие. Весь план собран из реальных отзывов, без LLM в рантайме.
