...
const DOSSIER_SLUGS = new Set([
  "yoga", "cosmetics-ingredient-checker",
...
const FLAW_BUCKETS: { label: string; color: string; kws: string[] }[] = [
  ...
  { label: "Точность и баги", color: "#9aa0a6", kws: ["подсчёт", "подсчет", "счита", "сбива", "застрева", "отмеч", "вис", "перелист", "точн", "глюк", "баг", "ошиб", "вылет", "крэш"] },
            ...
            <div className="mt-12 flex flex-col gap-12 sm:gap-16">
              ...
              {overviewUnlocked ? (
                ...
                <div className="pt-2">
                  ...
                  <CategoryOffer slug={slug} categoryName={cat.name} sellable={sellable} price={catPrice} loggedIn={loggedIn} pregenDate={pregenDate} locale={locale} ideasCount={opps.length} appsCount={apps.length} />
                </div>
              )}
            </div>
          </section>
