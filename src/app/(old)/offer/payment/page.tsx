...
// Public offer (публичная оферта) for purchases on the inapp.pro website —
// required by ЮKassa for accepting card payments. Requisites come from
// src/data/legal.json. The Russian text is the legal document itself and is
// kept as-is on both locales; /en only gets a one-line English note on top.
...
export default async function PaymentOfferPage() {
  ...
  const sections: Array<[string, React.ReactNode]> = [
    ...
    [
      ...
      <>
        ...
        сервис ЮKassa (ООО НКО «ЮМани»). Цены указаны в рублях РФ.
      </>,
    ],
    [
  ...
  return (
    <main lang="ru" className="mx-auto w-full max-w-[720px] px-4 py-14">
      {locale === "en" && (
        <p lang="en" className="mb-4 text-callout text-[var(--color-text-secondary)]">
          Public offer for purchases on the inapp.pro website (Russian-language document).
        </p>
      )}
