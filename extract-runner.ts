...
async function extractFile(inputPath: string) {
  ...
  try {
    ...
    console.error(`ERROR: ${filename} - invalid JSON in reviews: ${(e as Error).message}`);
  ...
  try {
    ...
    if (!parsed.results || !Array.isArray(parsed.results)) {
      console.error(
        `ERROR: ${filename} - response missing results[]`
      );
    ...
    console.error(
      `ERROR: ${filename} - ${(e as Error).message}`
    );
    ...
    const status =
      r.status === "skipped"
        ...
        : r.status === "written"
          ? "OK"
          : "ERR";
    const detail = (r as any).reviews
      ? ` (${(r as any).reviews} reviews)`
      : (r as any).reason
        ? ` (${(r as any).reason})`
        : "";
    console.log(
      `${status}: ${r.filename}${detail}`
    );
  }
}
